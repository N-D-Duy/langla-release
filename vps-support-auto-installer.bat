...
if %errorlevel% equ 0 (
    ...
    if %errorlevel% neq 0 (
        echo [ERROR] Loi khi tai file voi curl
        goto :error
    )
) else (
    echo Su dung PowerShell de tai file...
    powershell -Command "Invoke-WebRequest -Uri '%DOWNLOAD_URL%' -OutFile '%FILENAME%'"
    if %errorlevel% neq 0 (
        echo [ERROR] Loi khi tai file voi PowerShell
        goto :error
    )
)
...
if not exist "%FILENAME%" (
    echo [ERROR] File khong ton tai sau khi tai
    goto :error
)
...
if exist "%FULLPATH_FILENAME%" (
    ...
    echo Dang tu dong xoa file nay...
